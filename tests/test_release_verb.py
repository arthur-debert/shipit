...
@pytest.fixture
def bump_tools_on_path(monkeypatch):
    """Every adapter-provisioned tool present on PATH — the rust-leg tests'
    default, so the recorded calls are the adapters' alone regardless of the
    host machine (the self-provisioning path is pinned by its own test).
    Mirrors the bundle tests' ``cargo_deb_on_path``."""
    monkeypatch.setattr(release_verb.shutil, "which", lambda name: f"/stub/{name}")
...
def test_recorded_adapter_command_lines_per_leg(
    tmp_path, monkeypatch, bump_tools_on_path
):
...
def test_rust_leg_self_provisions_cargo_edit_when_missing(tmp_path, monkeypatch):
    """Issue #793 (the #784-F2/#785 pattern): the wf-prepare runner arrives
    without cargo-edit, so `cargo set-version` dies with rc=101 before the
    tag. When the probe (`cargo-set-version`) is off PATH, prepare installs
    the PINNED cargo-edit through the same recorded Exec seam, at the rust
    leg's cwd, BEFORE the adapter's commands — and no re-probe gates the
    commands afterward (cargo resolves custom subcommands via $CARGO_HOME/bin
    itself, independent of PATH; the #785 rounds-1–2 lesson)."""
    monkeypatch.setattr(release_verb.shutil, "which", lambda name: None)
    root = make_repo(
        tmp_path,
        monkeypatch,
        toml='[toolchains]\n"." = "rust"\n',
