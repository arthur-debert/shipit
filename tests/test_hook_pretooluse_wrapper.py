...
import pytest
from conftest import managed_bashguard_hook_command, managed_pretooluse_hook_command
...
def _worktree_add_payload() -> str:
    return json.dumps(
        {
            "tool_name": "Bash",
            "tool_input": {"command": "git worktree add ../t b"},
        }
    )
...
def _run_wrapper(
    ...
    return subprocess.run(
        ["bash", "-c", command or managed_pretooluse_hook_command()],
        cwd=cwd,
        env=env,
        input=payload or _coordinator_edit_payload(),
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def test_bashguard_wrapper_allows_when_pixi_is_entirely_absent(tmp_path):
    env = {"PATH": "/usr/bin:/bin", "CLAUDE_PROJECT_DIR": str(tmp_path)}
    result = _run_wrapper(
        tmp_path,
        env,
        command=managed_bashguard_hook_command(),
        payload=_worktree_add_payload(),
    )
    assert result.returncode == 0
    assert result.stdout == ""
...
def test_bashguard_wrapper_allows_on_any_nonzero_resolution_chain_exit(tmp_path):
    fake_bin = tmp_path / "fakebin"
    fake_bin.mkdir()
    stub = fake_bin / "pixi"
    stub.write_text(
        textwrap.dedent(
            """\
            #!/bin/sh
            echo "pixi: simulated resolution failure" >&2
            exit 1
            """
        )
    )
...
def test_bashguard_wrapper_passes_a_real_decided_guard_through_unchanged():
    if shutil.which("pixi") is None:
        pytest.skip("pixi not on PATH in this environment")
    if not (REPO_ROOT / ".pixi" / "envs" / "default").exists():
        pytest.skip("no provisioned default env — refusing to trigger a solve")
    ...
    decision = json.loads(result.stdout)["hookSpecificOutput"]
    assert decision["permissionDecision"] == "deny"
    assert "ADR-0014" in decision["permissionDecisionReason"]
