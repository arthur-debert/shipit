...
# ---------------------------------------------------------------------------
# Issue #1006 — a model UNUSABLE for a reviewer Run is declared on the identity
# and refused mechanically, not documented in a docstring nothing enforces.
# ---------------------------------------------------------------------------


def test_agy_declares_every_flash_tier_unusable_for_a_review_run():
    # The #1006 regression: `flash` was pinned as agy's reviewer model and every
    # run failed, because Gemini Flash goes agentic in `--print` and narrates
    # instead of answering. The hazard is now DECLARED data on the ONE identity.
    b = backend.ANTIGRAVITY
    assert set(b.review_unusable_models) == {
        "Gemini 3.5 Flash (High)",
        "Gemini 3.5 Flash (Low)",
    }
    # Declared by VERBATIM id, so BOTH the alias and its resolved name are caught
    # by the same entry — a new alias onto a known-bad model cannot sneak past.
    assert b.review_model_refusal("flash") is not None
    assert b.review_model_refusal("Gemini 3.5 Flash (High)") is not None
    assert b.review_model_refusal("flash_lite") is not None
    # The capable default is NOT refused — the reviewer still runs on `pro`.
    assert b.review_model_refusal("pro") is None
    assert b.review_model_refusal() is None  # the default resolves + passes


def test_require_review_model_refuses_a_known_unusable_model_loudly():
    with pytest.raises(ValueError) as exc:
        backend.ANTIGRAVITY.require_review_model("flash")
    message = str(exc.value)
    assert "agy" in message  # names the reviewer
    assert "flash" in message and "Gemini 3.5 Flash (High)" in message  # both forms
    assert "agentic" in message  # the REASON, not a generic refusal
    assert "'pro'" in message  # the capable default to switch to
    # The refusal must NOT send the operator chasing size/latency (#1006).
    assert "smaller diff" not in message


def test_require_review_model_passes_a_usable_model_and_is_a_guard_not_a_resolver():
    # A capable model passes silently — the guard returns nothing; resolving a
    # model for launch stays the adapter's job.
    assert backend.ANTIGRAVITY.require_review_model("pro") is None
    assert backend.ANTIGRAVITY.require_review_model() is None  # the default
    # A backend with NO declared hazards accepts anything.
    assert backend.CODEX.require_review_model("flash") is None
    assert backend.CLAUDE.review_unusable_models == {}


def test_a_backend_with_no_default_model_is_not_refused_for_having_none():
    # `claude` (a legal calibrator backend) has no default model AND no declared
    # hazards: the guard must short-circuit, not raise `no default model` and
    # turn "nothing known-bad here" into a spurious reviewer refusal.
    assert backend.CLAUDE.default_model is None
    assert backend.CLAUDE.review_model_refusal(None) is None
    assert backend.CLAUDE.require_review_model(None) is None
