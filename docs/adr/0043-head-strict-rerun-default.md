...
requester of required reviewers (ADR-0031), and rounds after the first review
only the fix range (`last-reviewed-head..new-head`) as a cheap single
...
Consequence: the review-once-default rule written into `docs/dev-cycle.lex`
(release repo — the canonical dev-cycle doc), the global CLAUDE.md guidance,
and shipit's role docs must be updated in the same change; the canonical doc
wins on drift, so it flips first.
