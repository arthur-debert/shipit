...
- **Native reviewer agent.** The reviewer posture (`read_only=True` AND `role == reviewer`) now
  emits **`--agent reviewer`** and passes the review task **directly** — no `role_prompt` prepend.
  The def is a **slim** reviewer agent shipit generates from the reviewer role OVERLAY (not the
  full base+overlay prompt) with only the **documented AGY frontmatter** (`name`, `description`)
  plus focused reviewer instructions. It is generated by `shipit.harness.prompts.regenerate`
  (`pixi run regen-roles`), force-included into the wheel, delivered by `shipit install` as a
  managed unit, and provisioned offline by `shipit.review.replay.provision_agent_defs` — the same
  four-surface path the Claude agent-defs use, so a bare experiment clone and an installed consumer
  both carry it. Because the posture is now flag-dependent, a real reviewer launch **preflights the
  capability** (`antigravity.require_agent_support`) and fails with a clean *upgrade agy* message if
  the installed CLI predates `--agent`, never a confusing mid-run "unknown option".
