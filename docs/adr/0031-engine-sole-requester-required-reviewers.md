...
`shipit install` retired-files mechanism), the managed ruleset template pins
`automatic_copilot_code_review_enabled: false` so `gh-setup`'s full-PUT wipes
any hand-edit, and the account-level Copilot auto-review setting is switched
off. No engine change is needed to take over: `CopilotAdapter.request()`
...
- **Ruleset-level automatic Copilot review** (`pull_request` rule parameter) —
  ...
  instead of a versioned workflow file. The template now pins it false.
