...
> as `noarch`") are no longer deferred: they ride the channel as
> `noarch: generic` conda packages through the same `conda` derived endpoint,
> extended with a noarch mode. See ADR-0076.
