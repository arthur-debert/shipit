...
| `wasm-pack` | bundle (wasm-pack composition, TOL02-WS12 #788) | pixi-managed (`pixi.toml#shipit-rust-release-deps`, rust signal — on conda-forge, which pulls the wasm32-unknown-unknown target std; WS10 #798) | `0.15.*` (#846: conda-forge never carried a 0.13 build) | `test_pins_agree_with_their_one_authority` (pin lockstep) |
