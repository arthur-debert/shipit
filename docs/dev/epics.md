...
Epic-branch currency is MERGE-only: a WS branch takes the new epic head by merging `EPIC/umbrella` in, NEVER by rebasing onto it. Re-review rounds are head-strict incremental — each round reviews only `last-reviewed-head..new-head` (ADR-0043) — and a rebase rewrites the WS branch's history so the last-reviewed head is no longer an ancestor of the new one: the next round's range degenerates to the whole umbrella delta, and reviewers re-flag sibling workstreams' already-landed code as if it were this PR's (observed live on \#732; confirmed by the maintainer mid-epic). Merging keeps the range to the merge commit plus the genuine fixes, and the squash-merge at integration erases the merge commits anyway — the merge-commit noise never outlives the WS PR.
