...
1. What pixi is to shipit
    ...
    What Work Env adds above pixi:
        ...
        - `pixi-run` for provisioned write Trees, CI Lane jobs, and provisioned
          fleet-sweep cells;
        - `activation-snapshot` for coordinator session Trees that borrowed
          `pixi shell-hook --json`; and
        - `ambient` for reviewers, explorers, Main checkouts without a supplied
