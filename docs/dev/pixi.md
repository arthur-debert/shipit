...
STILL OPEN, by design: the reviewer read-only Tree launch stays bare (no `.pixi/envs/default` to route into — a `chmod`'d clone). Tolerable because a reviewer only needs `gh` off the ambient PATH; it becomes a latent wrong-env bug only if a reviewer ever needs a Tree-pinned tool. The decision is centralised in the `pixi_wrap` gate, so a future "provision a read-only pixi env" change flips it in one place.
