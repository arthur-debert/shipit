...
1. The single-task cycle (one PR)
    ...
    Engine-owned policy — trust the tool, don't carry it in your head:
        ...
        (per-reviewer, default head-strict `rerun=true` — every push re-stales a
        required reviewer and the engine re-requests it, so the commits that
        address a round are themselves reviewed; review-once is an explicit
        per-reviewer opt-out), and the stopping breaker (stop at the configured
        round cap — `round_cap` in `[reviewers]` of `.shipit.toml`, default 6 —
        or on a round with NO major-or-worse *Finding*: minor and nit findings
        still resolve their threads before Ready but never mint another round).
        Findings arrive pre-classified on the 4-tier severity ladder, so the
        engine routes by severity directly and no role re-classifies. Round 1 is
        the exhaustive pass (each local reviewer fans its review into parallel
        dimension passes whose deduped union is posted); rounds after the first
        review only the fix range (`last-reviewed-head..new-head`) as one cheap
        incremental pass, new nits suppressed — the review loop converges by
        construction. Do what the engine reports rather than re-deriving these;
        on break it routes straight to READY and suppresses all re-requests (so a
        final nit-fix push cannot reopen the loop), and a real blocker (failing
...
2. Epics (multiple PRs)
    ...
    A feature is planned before execution via `/planning` (ideation -> Spec
    -> ADRs -> docs PR, then epic/WS decomposition -> issues).
