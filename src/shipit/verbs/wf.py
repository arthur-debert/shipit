...
fidelity, and dispatch UX are outside act's reach, and the standing notice is
...
#: a branch, an opened pull request, a dispatch with inputs. Extending it is a
#: registry entry in :func:`craft_event`, never a caller-side payload.
EVENT_PUSH = "push"
...
def craft_event(
    ...
    (base ``main``) pull request, and the dispatch ref. ``inputs`` feed
    ``workflow_dispatch`` alone; the CALLER enforces that scoping (the verb
    refuses ``--input`` on other kinds rather than dropping it silently).
    """
    if kind == EVENT_PUSH:
        return {
    ...
    if kind == EVENT_WORKFLOW_DISPATCH:
        return {
            "ref": f"refs/heads/{branch}",
...
def act_argv(
    *,
    ...
    rides only when a selector was given.
    """
    argv = [
        "act",
        event,
...
def run(
    ...
    *,
    ...
    if inputs and event != EVENT_WORKFLOW_DISPATCH:
        return _fail(
            f"--input only applies to --event {EVENT_WORKFLOW_DISPATCH} "
            f"(got --event {event})"
        )
    try:
    ...
    with tempfile.TemporaryDirectory(prefix="shipit-wf-") as tmp:
        ...
        argv = act_argv(event=event, workflow=workflow, event_path=event_path, job=job)
...
@click.option(
    ...
    multiple=True,
    metavar="KEY=VALUE",
    help="A workflow_dispatch input (repeatable; only with --event workflow_dispatch).",
)
def test_cmd(
    workflow: str, job: str | None, event: str, branch: str, inputs: tuple[str, ...]
) -> None:
    ...
    payload. Every run prints the act-untestable surface — the part of CI only
    a real push can verify. Exits 0 on a green run, 1 on a failed one; a
    missing act or docker fails hard, it never skips.
    """
    raise SystemExit(run(workflow, job=job, event=event, branch=branch, inputs=inputs))
