...
#: The Conda ACTIVATION vars only; installation-level ``CONDA_*`` is kept, since
#: scrubbing it could break ``pixi run`` itself in a Conda-managed shell.
CONDA_ACTIVATION_VARS = frozenset(
    {"CONDA_PREFIX", "CONDA_DEFAULT_ENV", "CONDA_SHLVL", "CONDA_PROMPT_MODIFIER"}
)

#: The per-Tree keys pixi ``[activation.env]`` re-sets, which an inherited value would
#: shadow. ``RUSTC_WRAPPER`` / ``SCCACHE_*`` are kept: dropping them disables sccache.
...
def is_leaked_env_var(key: str) -> bool:
    """Whether ``key`` is a parent-project env pointer; every scrub path relies solely on this."""
    ...
    if key.startswith("CONDA_ENV_SHLVL_"):
        # Keeping these restore keys while `CONDA_SHLVL` is scrubbed leaves a
        # half-scrubbed stack, against which pixi's nested activation mis-diffs.
        return True
