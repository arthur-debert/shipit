...
def format_lefthook_conflict(conflict: LefthookConflict) -> str:
    ...
    return (
        ...
        f"the '{conflict.hook}' hook and this repo's {conflict.local_path} sets "
        f"{named(conflict.local_options)} — lefthook refuses a merged hook with "
        f"both 'piped' and 'parallel' true and crashes BEFORE running any check, "
        f"blocking every git operation that fires '{conflict.hook}'. Remove the "
        f"option from {conflict.local_path} (the managed {LEFTHOOK_FILE} is "
        f"regenerated by `shipit install` — never edit it), then re-run."
    )
