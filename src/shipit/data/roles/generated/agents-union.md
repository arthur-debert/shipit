...
- Keep the ONE-LINE contract of what you changed accurate — that is the whole docstring duty, because a docstring that contradicts the code is the code lying to the next reader. Do NOT update caller docstrings, sibling modules, or module docstrings unless your change actually invalidates a sentence written there; a one-line code change is a one-line prose change. Code prose says WHAT a thing does and any contract a caller cannot infer from the name and signature — never rationale, ADR or issue references, epic codes, history ("was X, now Y"), or design narrative. Why lives in `docs/adr/`, vocabulary in `CONTEXT.md`, process in `docs/dev/`, history in git and the tracker; each idea has exactly ONE home (ADR-0079). Terse is correct: a function whose name and signature already say what it does needs no docstring at all.
...
- Writing planning docs — Specs, ADRs, CONTEXT.md — yourself; planning is NOT implementation, so the edit guard allows it.
- Promoting durable learnings INTO THE REPO before wrapping up — at end of epic and end of session alike. Route each learning to its proper repo home: a process rule -\> the relevant role .lex (then `pixi run regen-roles`) or docs/dev/; a decision -\> an ADR; vocabulary -\> CONTEXT.md; an open investigation -\> a tracker issue. Do this because those artifacts are how the REPO carries knowledge to everyone who reads it — reviewers, future implementers, humans — not because memory leaks. Session memory now persists: it lives in the repo's Session store, shared by every Tree of the repo and outliving all of them (ADR-0073), so it is a genuine archive for session-scoped context — what you tried, what the human decided mid-flight, where a thread left off. It is NOT a substitute for the repo artifacts above: a decision that only you remember is a decision the next reviewer cannot see. Write memory freely; promote the durable parts anyway.
...
- That clause cuts BOTH ways on code prose (ADR-0079): a missing, terse, or one-line docstring is NEVER a finding — never ask an author to expand, restore, or "document" prose — and prose that adds rationale, ADR or issue references, epic codes, history ("was X, now Y"), or design narrative to code IS a finding, because that content belongs in `docs/adr/`, `CONTEXT.md`, `docs/dev/`, or the tracker. The one exception is a single `See docs/adr/NNNN-slug.md` pointer in the module docstring of a module that directly implements that ADR. A docstring that CONTRADICTS the code it describes stays a finding at its own severity — that is a correctness defect, not style.
