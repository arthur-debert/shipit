...
- Verify commands: {{verify-commands}} — the EXACT commands that prove a round's fixes good BEFORE the push: the manual verify is the repo's test suite (in shipit: `pixi run test`), plus every role-relevant gotcha spelled out (in shipit: the lint gate — `pixi run lint`, the same task and pinned environment the hooks and CI run — is exercised by the commit/push git hooks, NOT run as a separate verify step; run `pixi run lint --fix` manually only when you expect formatting damage, then let the hook be the check; a `.lex` edit under `src/shipit/data/roles/` regenerates its derived surfaces via `pixi run regen-roles`, and the mirrors commit WITH the source). Name them exactly — a shepherd must never have to guess how to verify.
...
- Decision boundaries: {{decision-boundaries}} — what is already decided and must NOT be re-litigated: a finding that re-opens one of these gets a rationale reply, then the thread is resolved; never make a fix that unwinds the decision.
