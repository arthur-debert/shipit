...
- Verify commands: {{verify-commands}} — the EXACT commands that prove a round's fixes good BEFORE the push: the repo's test suite and lint gate (in shipit: `pixi run test` and `pixi run -e lint lint`), plus every role-relevant gotcha spelled out (in shipit: the lint gate is CHECK-ONLY — it reports formatting damage, `shipit lint --fix` repairs it; a `.lex` edit under `src/shipit/data/roles/` regenerates its derived surfaces via `pixi run regen-roles`, and the mirrors commit WITH the source). Name them exactly — a shepherd must never have to guess how to verify.
- Governing docs: {{governing-docs}} — the epic's governing ADR/PRD list this PR answers to; the shepherd self-checks each round's diff against each named doc BEFORE pushing the round.
