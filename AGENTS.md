...
The coordinator CREATES the branch off `main` (`fix/<issue>`) and spawns an IMPLEMENTER to do the task + tests. The implementer runs the gate (`shipit lint`) and tests (`pixi run test`) green BEFORE opening the PR — CI runs the same as required checks, so local green is necessary for CI green.
