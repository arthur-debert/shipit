...
1. **pixi is the parent of provisioning + hooks — NOT the agent session.**
   `pixi install` provisions a Tree; every Claude Code hook fires as
   `pixi run shipit hook <name>`. But `shipit spawn subagent` launches `claude -p`
...
2. **There is no pixi run-id / env-UUID / on-disk log.** pixi persists only
   *static* state (envs by name; `.pixi/envs/<env>/conda-meta/.pixi-environment-fingerprint`
   ...
   per-run correlation key is Claude Code's `session_id`** (the transcript
...
4. **Known P0 bug: a spawned agent runs OUTSIDE its Tree's pixi env.**
   ...
   `pixi run --manifest-path <tree>/pixi.toml --clean-env claude -p …`
